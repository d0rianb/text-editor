...
impl Range {
    ...
    pub fn get_lines_index(&mut self, lines: &[Line]) -> Vec<(u32, u32)> {
        ...
        for y in start.y ..= end.y {
            if y == start.y {
                if start.y == end.y { result.push((start.x, end.x)) }
                else { result.push((start.x, get_line_length(y, lines))) }
            }
            else if y == end.y { result.push((0, end.x))}
            else { result.push((0, get_line_length(y, lines))) }
        }
        result
    }
